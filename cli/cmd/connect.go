  ...
  unlarp connect --shell zsh  # Força shell específico`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if connectUseTmux {
			...
			if connectClaudeConfigDir != "" {
				// -e só afeta a criação; se a sessão já existe o new-session é no-op
				newSessionFlags += " -e " + shellQuote("CLAUDE_CONFIG_DIR="+connectClaudeConfigDir)
			}
...
func init() {
	...
	connectCmd.Flags().StringVar(&connectClaudeConfigDir, "claude-config-dir", "", "CLAUDE_CONFIG_DIR injetado na sessão Tmux (apenas na criação)")
}
